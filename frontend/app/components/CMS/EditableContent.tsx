...
export function EditableContent({
  ...
  if (windowWidth !== undefined && contentObj?.customStyleEnabled && contentObj.styles) {
  ...
  // If this is a plain slot (rendered as a span, p, h1, etc.), strip block-level tags 
  // generated by the rich text editor to prevent HTML nesting hydration mismatches
  // (e.g. an <h6> inside an <h2> or a <p> inside a <span>).
  if (plain) {
    // Strip ALL block-level tags that are invalid inside a <p> or inline element.
    // Leaving <ul>/<ol>/<li> inside a <p> causes browsers to auto-correct the DOM,
    // producing a server/client HTML mismatch on hydration.
    html = html.replace(/<\/?(p|div|h[1-6]|ul|ol|li|blockquote|pre|table|thead|tbody|tr|th|td)[^>]*>/gi, "");
  }
